if Customer.all.count == 0
  ...
  State.create!(name: "Minnesota", code: "MS")
  ...
  State.create!(name: "West Virgina", code: "WV")
  ...
  def create_billing_address(customer_id, num_states)
    ...
    CustomerBillingAddress.create!(customer_id: customer_id, address: billing_address)
  end
  ...
  def create_shipping_address(customer_id, num_states, is_primary)
    ...
    CustomerShippingAddress.create!(customer_id: customer_id, address: shipping_address, primary: is_primary)
  end

  # Cache the number of states so we don't have to query each time thru
